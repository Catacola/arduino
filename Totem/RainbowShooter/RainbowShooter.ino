...
void setup() {
  ...
  for(int i=0; i<N_STRIPS; i++) {
    strip[i] = Adafruit_NeoPixel(STRIP_LENGTH, FIRST_STRIP_PIN + i, NEO_GRB + NEO_KHZ800);
    strip[i].begin();
  ...
  for (int i = 0; i < STRIP_LENGTH; i++) {
    if (i <= frontPixel) {
      ...
      if (numBack > TAIL_LENGTH) {
        setEachPixelColor(i, 0);
      } else {
        float fadeFactor = pow(TAIL_FALL_OFF, numBack);
        setEachPixelColor(
          i,
          strip[0].Color(FLASH_R*fadeFactor, FLASH_G*fadeFactor, FLASH_B*fadeFactor)
        );
      }
    } else {
      setEachPixelColor(i, 0);
...
void setEachPixelColor(int pixel, uint32_t c) {
  for(int i=0; i<N_STRIPS; i++){
